...
namespace Engine.Services
{
    ...
    {
        private static GameDetails _gameDetails { get; set; }

        public static GameDetails ReadGameDetails()
        {
            JObject gameDetailsJson = JObject.Parse(File.ReadAllText
                ("D:\\C# WPF RPG\\SOSCSRPG\\Engine\\GameData\\GameDetails.json"));
            ...
            {
                gameDetails.PlayerAttributes.Add(new
                    PlayerAttribute(token.StringValueOf("Key"), token.StringValueOf("DisplayName"),
            ...
            if (gameDetailsJson["Races"] != null)
            {
                foreach (JToken token in gameDetailsJson["Races"])
                {
                    ...
                    if (token["PlayerAttributeModifiers"] != null)
                    {
                        ...
                        {
                            race.PlayerAttributeModifiers.Add(new
